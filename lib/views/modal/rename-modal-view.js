...
export default class RenameModalView extends ViewBase {
  ...
  executeRename(event) {
    ...
    fse.move(`${this.basePath}/${originPath}`, `${this.basePath}/${destinationPath}`, (error) => {
      ...
      ['qiita', 'esa', 'docbase', 'confluence', 'backlog'].forEach((serviceName) => {
        ...
        if (this.isDirectory) {
          ...
          // In case of file.
          const { itemId } = this.config.findItemByPath(serviceName, originPath)
          if (itemId && itemsConfig[itemId]) itemsConfig[itemId].path = this.config.relativizeItemPath(destinationPath)
        }
