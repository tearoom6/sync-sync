...
export default class EsaSectionView extends SectionViewBase {
  ...
  handleEsaEvent(event, type = 'import') {
    try {
      ...
      EsaSectionView.startHandlingEsaEvent(type, configPath, localPath, accessToken, teamName, esaTitle, esaTags, esaCategory)
    } catch (error) {
  ...
  static startHandlingEsaEvent(type, configPath, localPath, accessToken, teamName, esaTitle, esaTags, esaCategory) {
    if (localPath === '') {
        ...
        handledProcess = esaService.listAllItems(teamName)
          .then((items) => {
            ...
            items.forEach((item) => {
              ...
              const file = new File(`${localPath}/${fileDirectory}${item.name}.md`)
              if (file.existsSync()) {
              ...
              const fileSaveProcess = file.write(item.body_md).then(() => {
        ...
        if (type === 'import') {
          ...
          handledProcess = esaService.getItem(teamName, itemId)
            .then((item) => {
              ...
              // Save file.
              return targetFile.write(item.body_md).then(() => {
