...
export default class WordpressOrgSectionView extends SectionViewBase {
  ...
  render() {
    return (
      <section className="wordpress-org service">
        <h2>Sync with WordPress.org</h2>
        ...
        <label htmlFor="wordpress-org-tags">
          ...
          >
            {
              (this.props.wordpressOrgTagLists || [])
                .map(
                  tag => (
        ...
        <label htmlFor="wordpress-org-categories">
          ...
          >
            {
              (this.props.wordpressOrgCategoryLists || [])
                .map(
                  category => (
  ...
  importFromWordpressOrg(event) {
    this.handleWordpressOrgEvent(event, 'import')
  }

  exportToWordpressOrg(event) {
    this.handleWordpressOrgEvent(event, 'export')
  }

  handleWordpressOrgEvent(event, type = 'import') {
    try {
      ...
      const wordpressOrgTags = Array.from(this.refs.wordpressOrgTags.selectedOptions).map(option => option.value).join(',')
      const wordpressOrgCategories = Array.from(this.refs.wordpressOrgCategories.selectedOptions).map(option => option.value).join(',')
  ...
  static startHandlingWordpressOrgEvent(
    ...
    Promise.all([loadConfigPromise, loadSecretConfigPromise]).then((configs) => {
      ...
      const itemsConfig = config.prop('wordpressOrg', 'items') || {}
      ...
      if (fs.isDirectorySync(localPath)) {
        ...
        if (type === 'import') {
          ...
          //
          const { itemId } = config.findItemByPath('wordpressOrg', localPath, MatchType.exceptExport)
          if (!itemId) {
          ...
          // Call get API.
          handledProcess = wordpressOrgService.getItem(itemId)
            .then((item) => {
          ...
          handledProcess = targetFile.read().then((fileBody) => {
            ...
            const { itemId } = config.findItemByPath('wordpressOrg', localPath, MatchType.exceptImport)
            if (!itemId) {
              // In case of posting.
              // Call post API.
              return wordpressOrgService.postItem(title, fileBody, wordpressOrgTags, wordpressOrgCategories)
                .then((item) => {
            ...
            // In case of updating.
            // Call update API.
            return wordpressOrgService.updateItem(itemId, title, fileBody, wordpressOrgTags, wordpressOrgCategories)
              .then((item) => {
                // Save meta data to config.
                itemsConfig[item.id].updatedAt = item.modified_gmt
                itemsConfig[item.id].publishedAt = item.date_gmt
      ...
      handledProcess.then(() => {
        // Save meta data to config.
        config.setProp('wordpressOrg', 'items', itemsConfig)
