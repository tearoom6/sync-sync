...
export default class ConfluenceSectionView extends SectionViewBase {
  ...
  importFromConfluence(event) {
    this.handleConfluenceEvent(event, SyncType.import)
  }

  exportToConfluence(event) {
    this.handleConfluenceEvent(event, SyncType.export)
  }

  handleConfluenceEvent(event, type = SyncType.import) {
    try {
  ...
  static startHandlingConfluenceEvent(
    ...
    const loadConfigPromise = Config.load(configFile).then(config => {
      config.setProp(Services.confluence, 'userName', userName)
      config.setProp(Services.confluence, 'baseUrl', baseUrl)
      config.setProp(Services.confluence, 'space', space)
      return config
    })
    const loadSecretConfigPromise = Config.load(secretConfigFile).then(config => {
      if (atom.config.get('sync-sync.keepSecrets')) {
        config.setProp(Services.confluence, 'accessToken', accessToken)
      } else {
        config.deleteProp(Services.confluence, 'accessToken')
      }
      return config
    })
    ...
    Promise.all([loadConfigPromise, loadSecretConfigPromise])
      .then(configs => {
        ...
        const itemsConfig = config.prop(Services.confluence, 'items') || {}
        ...
        if (fs.isDirectorySync(localPath)) {
          ...
          handledProcess = confluenceService.listAllItems(space, confluenceType, items => {
            ...
            items.forEach(item => {
              ...
              itemsConfig[item.id] = itemsConfig[item.id] || { userName, baseUrl, space }
              itemsConfig[item.id].url = `${baseUrl}${item._links.webui}`
              ...
              if (!markdownImport) {
                ...
                itemsConfig[item.id].pathOnlyImport = config.relativizeItemPath(file.getPath())
              }
          ...
          if (type === SyncType.import) {
            ...
            //
            const { itemId } = config.findItemByPath(Services.confluence, localPath, MatchType.exceptExport)
            if (!itemId) {
            ...
            // Call get API.
            handledProcess = confluenceService.getItem(itemId).then(item => {
            ...
            handledProcess = targetFile.read().then(fileBody => {
              ...
              const { itemId } = config.findItemByPath(Services.confluence, localPath, MatchType.exceptImport)
              if (!itemId) {
                // In case of posting.
                // Call post API.
                return confluenceService
                  .postItem(space, title, fileBody, confluenceType, confluenceParentId, confluenceFormat)
                  .then(item => {
              ...
              // Call update API.
              return confluenceService
                .updateItem(itemId, title, fileBody, confluenceType, itemsConfig[itemId].version + 1, confluenceParentId, confluenceFormat)
                .then(item => {
                  // Save meta data to config.
                  const parentItem = item.ancestors[item.ancestors.length - 1]
                  itemsConfig[item.id].updatedAt = item.version.when
        ...
        handledProcess
          .then(() => {
            // Save meta data to config.
            config.setProp(Services.confluence, 'items', itemsConfig)
