  ...
  static startHandlingDocBaseEvent(
    ...
    Promise.all([loadConfigPromise, loadSecretConfigPromise])
      .then(configs => {
        ...
        const itemsConfig = config.prop(Services.docbase, 'items') || {}
        ...
        if (fs.isDirectorySync(localPath)) {
          ...
          handledProcess = docbaseService.listAllItems(domain, items => {
            ...
            items.forEach(item => {
              const filePath = this.resolveDocBaseItemPath(localPath, configDir.getPath(), item, itemsConfig[item.id], optionKeepFilePath)
          ...
          if (type === SyncType.import) {
            ...
            //
            const { itemId } = config.findItemByPath(Services.docbase, localPath, MatchType.exceptExport)
            if (!itemId) {
            ...
            // Call get API.
            handledProcess = docbaseService.getItem(domain, itemId).then(item => {
            ...
            handledProcess = targetFile.read().then(fileBody => {
              ...
              const { itemId } = config.findItemByPath(Services.docbase, localPath, MatchType.exceptImport)
              const tags = docbaseTags.split(',').map(tag => tag.trim())
              if (!itemId) {
                // In case of posting.
                // Call post API.
                return docbaseService.postItem(domain, title, fileBody, tags, docbaseGroups, docbaseScope).then(item => {
              ...
              // In case of updating.
              // Call update API.
              return docbaseService.updateItem(domain, itemId, title, fileBody, tags, docbaseGroups, docbaseScope).then(item => {
                // Save meta data to config.
                itemsConfig[item.id].createdAt = item.created_at
                itemsConfig[item.id].title = item.title
                itemsConfig[item.id].tags = item.tags.map(tag => tag.name)
        ...
        handledProcess
          .then(() => {
            // Save meta data to config.
            config.setProp(Services.docbase, 'items', itemsConfig)
