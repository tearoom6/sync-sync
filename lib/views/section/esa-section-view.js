      ...
      if (fs.isDirectorySync(localPath)) {
        ...
        handledProcess = esaService.listAllItems(teamName, (items) => {
          ...
          items.forEach((item) => {
            ...
            let file = new File(`${localPath}/${fileDirectory}${item.name}.md`)
            if (optionKeepFilePath && itemsConfig[item.number]) {
              file = new File(`${configDir.getPath()}/${itemsConfig[item.number].path}`)
            }
            if (file.existsSync()) {
