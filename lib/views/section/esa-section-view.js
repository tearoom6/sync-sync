...
export default class EsaSectionView extends SectionViewBase {
  ...
  static startHandlingEsaEvent(type, configDirPath, localPath, optionKeepFilePath, accessToken, teamName, esaTitle, esaTags, esaCategory) {
    if (localPath === '') {
    ...
    Promise.all([loadConfigPromise, loadSecretConfigPromise])
      .then(configs => {
        ...
        const itemsConfig = config.prop(Services.esa, 'items') || {}
        ...
        if (fs.isDirectorySync(localPath)) {
          ...
          handledProcess = esaService.listAllItems(teamName, items => {
            ...
            items.forEach(item => {
              const filePath = this.resolveEsaItemPath(localPath, configDir.getPath(), item, itemsConfig[item.number], optionKeepFilePath)
          ...
          if (type === SyncType.import) {
            ...
            //
            const { itemId } = config.findItemByPath(Services.esa, localPath, MatchType.exceptExport)
            if (!itemId) {
            ...
            // Call get API.
            handledProcess = esaService.getItem(teamName, itemId).then(item => {
            ...
            handledProcess = targetFile.read().then(fileBody => {
              ...
              const { itemId } = config.findItemByPath(Services.esa, localPath, MatchType.exceptImport)
              const tags = esaTags.split(',').map(tag => tag.trim())
              if (!itemId) {
                // In case of posting.
                // Call post API.
                return esaService.postItem(teamName, title, fileBody, tags, esaCategory).then(item => {
              ...
              // In case of updating.
              // Call update API.
              return esaService.updateItem(teamName, itemId, title, fileBody, tags, esaCategory).then(item => {
                // Save meta data to config.
                itemsConfig[item.number].updatedAt = item.updated_at
                itemsConfig[item.number].title = item.name
                itemsConfig[item.number].tags = item.tags
        ...
        handledProcess
          .then(() => {
            // Save meta data to config.
            config.setProp(Services.esa, 'items', itemsConfig)
