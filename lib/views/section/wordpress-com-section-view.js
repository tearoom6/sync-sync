    ...
    Promise.all([loadConfigPromise, loadSecretConfigPromise]).then((configs) => {
      ...
      if (fs.isDirectorySync(localPath)) {
        ...
        if (type === SyncType.import) {
          ...
          //
          const { itemId } = config.findItemByPath('wordpressCom', localPath, MatchType.exceptExport)
          if (!itemId) {
          ...
          handledProcess = targetFile.read().then((fileBody) => {
            ...
            const { itemId } = config.findItemByPath('wordpressCom', localPath, MatchType.exceptImport)
            if (!itemId) {
              // In case of posting.
              // Call post API.
