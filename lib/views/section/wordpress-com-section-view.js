...
export default class WordpressComSectionView extends SectionViewBase {
  ...
  importFromWordpressCom(event) {
    this.handleWordpressComEvent(event, SyncType.import)
  }

  exportToWordpressCom(event) {
    this.handleWordpressComEvent(event, SyncType.export)
  }

  handleWordpressComEvent(event, type = SyncType.import) {
    try {
  ...
  static startHandlingWordpressComEvent(
    ...
    const loadConfigPromise = Config.load(configFile).then(config => {
      config.setProp(Services.wordpressCom, 'site', site)
      // TODO For legacy implementation. To be removed.
      config.deleteProp(Services.wordpressCom, 'accessToken')
      return config
    })
    const loadSecretConfigPromise = Config.load(secretConfigFile).then(config => {
      if (atom.config.get('sync-sync.keepSecrets')) {
        config.setProp(Services.wordpressCom, 'accessToken', accessToken)
      } else {
        config.deleteProp(Services.wordpressCom, 'accessToken')
      }
      return config
    ...
    Promise.all([loadConfigPromise, loadSecretConfigPromise])
      .then(configs => {
        ...
        const itemsConfig = config.prop(Services.wordpressCom, 'items') || {}
        ...
        if (fs.isDirectorySync(localPath)) {
          ...
          handledProcess = wordpressComService.listAllItems(items => {
            ...
            items.forEach(item => {
              ...
              itemsConfig[item.ID] = itemsConfig[item.ID] || { site }
          ...
          if (type === SyncType.import) {
            ...
            //
            const { itemId } = config.findItemByPath(Services.wordpressCom, localPath, MatchType.exceptExport)
            if (!itemId) {
            ...
            // Call get API.
            handledProcess = wordpressComService.getItem(itemId).then(item => {
            ...
            handledProcess = targetFile.read().then(fileBody => {
              ...
              const { itemId } = config.findItemByPath(Services.wordpressCom, localPath, MatchType.exceptImport)
              if (!itemId) {
                // In case of posting.
                // Call post API.
                return wordpressComService.postItem(title, fileBody, wordpressComTags, wordpressComCategories).then(item => {
              ...
              // In case of updating.
              // Call update API.
              return wordpressComService.updateItem(itemId, title, fileBody, wordpressComTags, wordpressComCategories).then(item => {
                // Save meta data to config.
                itemsConfig[item.ID].updatedAt = item.modified
                itemsConfig[item.ID].publishedAt = item.date
                itemsConfig[item.ID].title = item.title
        ...
        handledProcess
          .then(() => {
            // Save meta data to config.
            config.setProp(Services.wordpressCom, 'items', itemsConfig)
