...
export default class QiitaSectionView extends SectionViewBase {
  ...
  static startHandlingQiitaEvent(type, configDirPath, localPath, optionKeepFilePath, accessToken, userName, qiitaTitle, qiitaTags) {
    if (localPath === '') {
      ...
      .then(configs => {
        ...
        if (fs.isDirectorySync(localPath)) {
          ...
          handledProcess = qiitaService.listAllItems(userName, items => {
            ...
            items.forEach(item => {
              const filePath = this.resolveQiitaItemPath(localPath, configDir.getPath(), item, itemsConfig[item.id], optionKeepFilePath)
          ...
          if (type === SyncType.import) {
            ...
            // Call get API.
            handledProcess = qiitaService.getItem(itemId).then(item => {
            ...
            handledProcess = targetFile.read().then(fileBody => {
              ...
              if (!itemId) {
                // In case of posting.
                // Call post API.
                return qiitaService.postItem(title, fileBody, tags).then(item => {
              ...
              // In case of updating.
              // Call update API.
              return qiitaService.updateItem(itemId, title, fileBody, tags).then(item => {
                // Save meta data to config.
                itemsConfig[item.id].updatedAt = item.updated_at
                itemsConfig[item.id].title = item.title
                itemsConfig[item.id].tags = item.tags.map(tag => tag.name)
