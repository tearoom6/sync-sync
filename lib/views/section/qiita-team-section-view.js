...
export default class QiitaTeamSectionView extends SectionViewBase {
          ...
          >
            {
              (this.props.qiitaTeamGroups || [])
                ...
                .map(group =>
                  <option value={group.url_name} selected={group.url_name === this.props.qiitaTeamGroupId}>{group.name}</option>)
            }
          </select>
        </label><br />
  ...
  importFromQiitaTeam(event) {
    this.handleQiitaTeamEvent(event, 'import')
  }

  exportToQiitaTeam(event) {
    this.handleQiitaTeamEvent(event, 'export')
  }

  handleQiitaTeamEvent(event, type = 'import') {
    try {
  ...
  static startHandlingQiitaTeamEvent(
    ...
    const loadConfigPromise = Config.load(configFile).then((config) => {
      config.setProp('qiitaTeam', 'teamId', teamId)
      config.setProp('qiitaTeam', 'userName', userName)
      // TODO For legacy implementation. To be removed.
      config.deleteProp('qiitaTeam', 'accessToken')
      return config
    })
    const loadSecretConfigPromise = Config.load(secretConfigFile).then((config) => {
      if (atom.config.get('sync-sync.keepSecrets')) {
        config.setProp('qiitaTeam', 'accessToken', accessToken)
      } else {
        config.deleteProp('qiitaTeam', 'accessToken')
      }
      return config
    ...
    Promise.all([loadConfigPromise, loadSecretConfigPromise]).then((configs) => {
      ...
      const itemsConfig = config.prop('qiitaTeam', 'items') || {}
      ...
      if (fs.isDirectorySync(localPath)) {
        ...
        handledProcess = qiitaTeamService.listAllItems(userName, (items) => {
          ...
          items.forEach((item) => {
            const filePath = this.resolveQiitaTeamItemPath(
              localPath,
              configDir.getPath(),
              item,
              itemsConfig[item.id],
        ...
        if (type === 'import') {
          ...
          //
          const { itemId } = config.findItemByPath('qiitaTeam', localPath, MatchType.exceptExport)
          if (!itemId) {
          ...
          // Call get API.
          handledProcess = qiitaTeamService.getItem(itemId)
            .then((item) => {
          ...
          handledProcess = targetFile.read().then((fileBody) => {
            ...
            const { itemId } = config.findItemByPath('qiitaTeam', localPath, MatchType.exceptImport)
            const tags = qiitaTeamTags.split(',').map(tag => tag.trim())
            if (!itemId) {
              // In case of posting.
              // Call post API.
              return qiitaTeamService.postItem(title, fileBody, tags, qiitaTeamGroupId, qiitaTeamCoediting)
                .then((item) => {
            ...
            // In case of updating.
            // Call update API.
            return qiitaTeamService.updateItem(itemId, title, fileBody, tags, qiitaTeamGroupId, qiitaTeamCoediting)
              .then((item) => {
                // Save meta data to config.
                itemsConfig[item.id].updatedAt = item.updated_at
                itemsConfig[item.id].title = item.title
      ...
      handledProcess.then(() => {
        // Save meta data to config.
        config.setProp('qiitaTeam', 'items', itemsConfig)
