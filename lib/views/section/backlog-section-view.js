...
export default class BacklogSectionView extends SectionViewBase {
  ...
  importFromBacklog(event) {
    this.handleBacklogEvent(event, SyncType.import)
  }

  exportToBacklog(event) {
    this.handleBacklogEvent(event, SyncType.export)
  }

  handleBacklogEvent(event, type = SyncType.import) {
    try {
      ...
      const domain = this.refs.backlogDomain.value
  ...
  static startHandlingBacklogEvent(
    ...
    const loadConfigPromise = Config.load(configFile).then(config => {
      config.setProp(Services.backlog, 'spaceKey', spaceKey)
      config.setProp(Services.backlog, 'domain', domain)
      return config
    })
    const loadSecretConfigPromise = Config.load(secretConfigFile).then(config => {
      if (atom.config.get('sync-sync.keepSecrets')) {
        config.setProp(Services.backlog, 'accessToken', accessToken)
      } else {
        config.deleteProp(Services.backlog, 'accessToken')
      }
      return config
    })
    ...
    Promise.all([loadConfigPromise, loadSecretConfigPromise])
      .then(configs => {
        ...
        const itemsConfig = config.prop(Services.backlog, 'items') || {}

        const backlogService = new BacklogService(spaceKey, domain, accessToken)
        let handledProcess
        ...
        if (fs.isDirectorySync(localPath)) {
          ...
          handledProcess = backlogService.listItems(selectedProject.id).then(listItems => {
            ...
            const eachItemPromise = listItem => {
              ...
              return backlogService.getItem(listItem.id).then(item => {
                ...
                itemsConfig[item.id].url = this.resolveBacklogItemUrl(spaceKey, domain, selectedProject, item)
            ...
            const runAfterDelayRecursively = delay =>
              new Promise(resolve => {
          ...
          if (type === SyncType.import) {
            ...
            //
            const { itemId } = config.findItemByPath(Services.backlog, localPath, MatchType.exceptExport)
            if (!itemId) {
            ...
            // Call get API.
            handledProcess = backlogService.getItem(itemId).then(item => {
            ...
            handledProcess = targetFile.read().then(fileBody => {
              ...
              const { itemId } = config.findItemByPath(Services.backlog, localPath, MatchType.exceptImport)
              if (!itemId) {
                // In case of posting.
                // Call post API.
                return backlogService.postItem(selectedProject.id, title, fileBody).then(item => {
              ...
              // In case of updating.
              // Call update API.
              return backlogService.updateItem(itemId, title, fileBody).then(item => {
                // Save meta data to config.
                itemsConfig[item.id].url = this.resolveBacklogItemUrl(spaceKey, domain, selectedProject, item)
                itemsConfig[item.id].updatedAt = item.updated
                itemsConfig[item.id].title = item.name
        ...
        handledProcess
          .then(() => {
            // Save meta data to config.
            config.setProp(Services.backlog, 'items', itemsConfig)
