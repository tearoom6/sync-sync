...
export default class SyncSyncView {
  ...
  static startImportFromQiita(configPath, localPath, accessToken, userName) {
    ...
    configFile.read().then((configBody) => {
      ...
      } else if (fs.isDirectorySync(localPath)) {
  ...
  exportToQiita(event) {
    try {
      ...
      SyncSyncView.startExportingToQiita(configPath, localPath, accessToken, userName, qiitaTags)
    } catch (error) {
      atom.notifications.addError('Something went wrong.')
      console.error('Error occurred in exportToQiita method: ', error)
    }
  }

  static startExportingToQiita(configPath, localPath, accessToken, userName, qiitaTags) {
    if (qiitaTags === '') {
      ...
      if (fs.isDirectorySync(localPath)) {
        ...
        targetFile.read().then((fileBody) => {
          ...
          if (!itemId) {
            // In case of posting.
            // Call post API.
            qiitaService.postItem(targetFile.getBaseName().replace(/\.[^/.]+$/, ''), fileBody, qiitaTags.split(','))
              .then((item) => {
                // Save meta data to config.
                itemsConfig[item.id] = {
                  path: relativeItemPath,
              ...
              .catch((error) => {
                atom.notifications.addError('Something went wrong.')
                console.error('Error occurred in qiitaService.postItem: ', error)
              })
          } else {
            // In case of updating.
            // Call update API.
            qiitaService.updateItem(itemId, targetFile.getBaseName().replace(/\.[^/.]+$/, ''), fileBody, qiitaTags.split(','))
              .then((item) => {
                // Save meta data to config.
                itemsConfig[item.id].updatedAt = item.updated_at
                itemsConfig[item.id].tags = item.tags.map(tag => tag.name)
