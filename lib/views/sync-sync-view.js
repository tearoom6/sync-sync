          ...
          .then((items) => {
            ...
            items.forEach((item) => {
              ...
              if (file.existsSync()) {
                ...
                if (!itemsConfig[item.id]) {
                  atom.notifications.addError(
                    'Cannot import because of not-synced local file.',
                    { detail: file.getPath() },
                  )
                  return
                }
                const modifiedAt = Date.parse(fs.statSync(file.getPath()).mtime)
                const lastSyncedAt = Date.parse(itemsConfig[item.id].syncedAt)
                if (modifiedAt > lastSyncedAt) {
                  atom.notifications.addError(
        ...
        if (type === 'import') {
          ...
          handledProcess = qiitaService.getItem(itemId)
            .then((item) => {
              if (targetFile.existsSync()) {
                ...
                const modifiedAt = Date.parse(fs.statSync(targetFile.getPath()).mtime)
                const lastSyncedAt = Date.parse(itemsConfig[item.id].syncedAt)
                if (modifiedAt > lastSyncedAt) {
                  atom.notifications.addError(
                    'Cannot import because of not-synced local modification.',
              ...
              itemsConfig[item.id].syncedAt = (new Date()).toISOString()
          ...
          handledProcess = targetFile.read().then((fileBody) => {
            ...
            if (!itemId) {
              // In case of posting.
              // Call post API.
              return qiitaService.postItem(fileName, fileBody, tags)
                .then((item) => {
            ...
            // In case of updating.
            // Call update API.
            return qiitaService.updateItem(itemId, fileName, fileBody, tags)
              .then((item) => {
                // Save meta data to config.
                itemsConfig[item.id].updatedAt = item.updated_at
                itemsConfig[item.id].syncedAt = (new Date()).toISOString()
