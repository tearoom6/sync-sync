      ...
      if (configFile) {
        ...
        Config.load(configFile).then((config) => {
          this.syncSyncView.update({
            ...
            docbaseItemUrl: null,
            wordpressComSite: config.prop('wordpressCom', 'site'),
            wordpressComItemUrl: null,
            wordpressOrgBaseUrl: config.prop('wordpressOrg', 'baseUrl'),
          ...
          const wordpressComItem = config.findItemByPath('wordpressCom', localPath).item
          if (wordpressComItem) {
            this.syncSyncView.update({
          ...
          const wordpressOrgItem = config.findItemByPath('wordpressOrg', localPath).item
          if (wordpressOrgItem) {
            this.syncSyncView.update({
              wordpressOrgTitle: wordpressOrgItem.title,
              wordpressOrgTags: wordpressOrgItem.tags.join(','),
              wordpressOrgCategories: wordpressOrgItem.categories.join(','),
      ...
      if (secretConfigFile) {
        ...
        Config.load(secretConfigFile).then((config) => {
          this.syncSyncView.update({
            ...
            wordpressComAccessToken: config.prop('wordpressCom', 'accessToken'),
            wordpressOrgAccessToken: config.prop('wordpressOrg', 'accessToken'),
