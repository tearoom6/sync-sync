...
export default {
  ...
  openMainView(event) {
    ...
    try {
      ...
      if (!secretConfigFile) {
        // Create new config file.
        secretConfigFile = new File(`${projectPath}/${ConfigUtil.getSecretConfigBaseName()}`)
        secretConfigFile.create().then((isCreated) => {
          secretConfigFile.writeSync('{}')
          console.log(`Secret config file is created: ${secretConfigFile.getPath()}`)
        })
      } else {
        // Load default value.
