...
export default class EsaService extends ServiceBase {
  ...
  async listAllItems(teamName, callback) {
    const listPartialItems = (page = 1, perPage = 100) =>
      this.listItems(teamName, page, perPage).then(items =>
        callback(items).then(() => {
  ...
  async listItems(teamName, page = 1, perPage = 100) {
    return this.callApi(`/v1/teams/${teamName}/posts?page=${page}&per_page=${perPage}&sort=created&order=asc`).then(response => {
      if (!response.ok) {
  ...
  async getItem(teamName, postId) {
    return this.callApi(`/v1/teams/${teamName}/posts/${postId}`).then(response => {
      if (!response.ok) {
        return response.json().then(error => {
