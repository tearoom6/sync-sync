...
export default class HatenaService extends ServiceBase {
  ...
  async listAllItems(callback) {
    const listPartialItems = async (page = '') => {
      ...
      await callback(result.items).then(() => {
        if (result.nextPage) {
  ...
  async listItems(page = '') {
    const responseText = await this.callApi(`/entry?page=${page}`).then(response => {
      if (!response.ok) {
  ...
  async getItem(itemId) {
    const responseText = await this.callApi(`/entry/${itemId}`).then(response => {
      if (!response.ok) {
  ...
  async postItem(
    ...
    const responseText = await this.callApi('/entry', 'POST', HatenaService.composePostBody(
  ...
  async updateItem(
    ...
    const responseText = await this.callApi(`/entry/${itemId}`, 'PUT', HatenaService.composePostBody(
      title, content, categories, isDraft,
    )).then(response => {
      if (!response.ok) {
