...
export default class HatenaService extends ServiceBase {
  ...
  async listAllItems() {
    const listPartialItems = async (alreadyFoundItems = [], page = '') => {
      const result = await this.listItems(page)
      const allItems = alreadyFoundItems.concat(result.items || [])
      if (result.nextPage) {
        return listPartialItems(allItems, result.nextPage)
      }
  ...
  async listItems(page = '') {
    ...
    return this.constructor.parseXmlResponse(responseText)
  }
  ...
  async postItem(
    ...
    const response = await this.callApi('/entry', 'POST', this.constructor.composePostBody(
      title, content, null, categories, isDraft,
    ))
    if (!response.ok) {
  ...
  async updateItem(
    ...
    const response = await this.callApi(`/entry/${itemId}`, 'PUT', this.constructor.composePostBody(
      title, content, updatedAt, categories, isDraft,
    ))
    if (!response.ok) {
    ...
    return new Promise((resolve, reject) => {
      new Parser({
        ...
        if (responseBody.feed) {
          ...
          resolve({
            items: wrappedArray(responseBody.feed.entry).map(item => this.convertItemFormat(item)),
            nextPage: nextLink ? nextLink.$.href.match(/\?page=(\d+)$/)[1] : null,
          })
        } else {
          resolve(this.convertItemFormat(responseBody.entry))
        }
      })
    })
  }
  ...
  static composePostBody(title, content, updatedAt = null, categories = [], isDraft = false) {
    const body = {
      entry: {
        $: {
