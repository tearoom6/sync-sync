    ...
    return new Promise((resolve, reject) => {
      new Parser({
        ...
        if (responseBody.feed) {
          ...
          resolve({
            items: wrappedArray(responseBody.feed.entry).map(item => this.constructor.convertItemFormat(item)),
            nextPage: nextLink ? nextLink.$.href.match(/\?page=(\d+)$/)[1] : null,
          })
        } else {
          resolve(this.constructor.convertItemFormat(responseBody.entry))
        }
      })
    })
  }
