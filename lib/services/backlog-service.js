...
export default class BacklogService extends ServiceBase {
  ...
  async listProjects() {
    return this.callApi('/api/v2/projects')
      .then((response) => {
        if (!response.ok) {
  ...
  async listItems(projectIdOrKey) {
    return this.callApi(`/api/v2/wikis?projectIdOrKey=${projectIdOrKey}`)
      .then((response) => {
        if (!response.ok) {
          if (response.headers.get('Content-Type') !== 'application/json') {
            return response.text().then((responseBody) => {
  ...
  async getItem(wikiId) {
    return this.callApi(`/api/v2/wikis/${wikiId}`)
      .then((response) => {
        if (!response.ok) {
          if (response.headers.get('Content-Type') !== 'application/json') {
            return response.text().then((responseBody) => {
  ...
  async postItem(projectId, name, content, mailNotify = false) {
    return this.callApi('/api/v2/wikis', 'POST', {
      projectId, name, content, mailNotify,
    }).then((response) => {
      if (!response.ok) {
        if (response.headers.get('Content-Type') !== 'application/json') {
          return response.text().then((responseBody) => {
