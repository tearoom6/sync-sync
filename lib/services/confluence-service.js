...
export default class ConfluenceService extends ServiceBase {
  ...
  async listAllItems(space, type = 'page', callback) {
    const listPartialItems = (page = 1, perPage = 25) =>
      this.listItems(space, type, page, perPage)
  ...
  async listItems(space, type = 'page', page = 1, perPage = 25, expand = 'body.storage,body.view,ancestors,version', depth = 'all') {
    return this.callApi(`/rest/api/space/${space}/content/${type}?`
      + `start=${(page - 1) * perPage}&limit=${perPage}&expand=${expand}&depth=${depth}`)
      .then((response) => {
        if (!response.ok) {
  ...
  async getItem(itemId, expand = 'body.storage,body.view,ancestors,version') {
    return this.callApi(`/rest/api/content/${itemId}?expand=${expand}`)
      .then((response) => {
        if (!response.ok) {
          return response.json().then((error) => {
  ...
  async postItem(
    space, title, body, type = 'page', parentItemId = null, format = 'storage',
    status = 'current', expand = 'body.storage,body.view,ancestors,version',
  ) {
    const params = {
      space: { key: space }, title, body: ConfluenceService.composeBody(body, format), type, status,
    }
  ...
  async updateItem(
    itemId, title, body, type, version, parentItemId = null, format = 'storage',
    status = 'current', conflictPolicy = 'abort', expand = 'body.storage,body.view,ancestors,version',
  ) {
    const params = {
      title, body: ConfluenceService.composeBody(body, format), type, version: { number: version }, status,
    }
    if (parentItemId && parentItemId !== '') params.ancestors = [{ id: parentItemId }]
