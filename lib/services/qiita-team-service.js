...
export default class QiitaTeamService extends ServiceBase {
  ...
  async listGroups() {
    return this.callApi('/api/v2/groups')
      .then((response) => {
        if (!response.ok) {
  ...
  async postItem(
    title, body, tags,
    groupUrlName = null, coediting = false, gist = false, tweet = false,
  ) {
    ...
    return this.callApi('/api/v2/items', 'POST', {
      title, body, tags: tagsObj, group_url_name: group, coediting, gist, tweet,
    }).then((response) => {
      if (!response.ok) {
        return response.json().then((error) => {
  ...
  async updateItem(
    itemId, title, body, tags,
    groupUrlName = null, coediting = false,
  ) {
    const tagsObj = tags.map(tag => QiitaTeamService.composeTag(tag))
    const group = (groupUrlName && groupUrlName.length !== 0) ? groupUrlName : null
    return this.callApi(`/api/v2/items/${itemId}`, 'PATCH', {
      title, body, tags: tagsObj, group_url_name: group, coediting,
    }).then((response) => {
      if (!response.ok) {
        return response.json().then((error) => {
