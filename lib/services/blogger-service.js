...
export default class BloggerService extends ServiceBase {
  ...
  async listAllItems() {
    const listPartialItems = async (alreadyFoundItems = [], page = null) => {
      const response = await this.listItems(page)
      const allItems = alreadyFoundItems.concat(response.items || [])
      if (response.nextPageToken) {
        return listPartialItems(allItems, response.nextPageToken)
      }
