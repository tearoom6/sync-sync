...
export default class BloggerService extends ServiceBase {
  ...
  async listAllItems(callback) {
    const listPartialItems = async (page = null) => {
      ...
      await callback(response.items || []).then(() => {
  ...
  async listItems(page = null) {
    return this.callApi(`/blogs/${this.blogId}/posts?fetchBodies=true${page ? `&pageToken=${page}` : ''}`).then(response => {
      if (!response.ok) {
  ...
  async getItem(itemId) {
    return this.callApi(`/blogs/${this.blogId}/posts/${itemId}`).then(response => {
      if (!response.ok) {
        return response.json().then(error => {
  ...
  async postItem(title, body, labels) {
    return this.callApi(`/blogs/${this.blogId}/posts/`, 'POST', {
      kind: 'blogger#post',
      title,
      content: body,
      labels,
  ...
  async updateItem(itemId, title, body, labels) {
    return this.callApi(`/blogs/${this.blogId}/posts/${itemId}`, 'PATCH', {
      title,
      content: body,
      labels,
    }).then(response => {
  ...
  async callApi(path, method = 'GET', body = null, additionalHeaders = {}, restOptions = {}) {
    ...
    return BloggerService.accessHttp(
