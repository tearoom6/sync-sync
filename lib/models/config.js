...
export default class Config {
  static async load(configFile) {
    if (configFile.existsSync()) {
      const configBody = await configFile.read()
  ...
  findItemsByDirectoryPath(serviceName, directoryPath) {
    ...
    const items = this.prop(serviceName, 'items')
    if (items) {
      const itemIds = Object.keys(items).filter(key => items[key].path.startsWith(`${relativeDirPath}/`))
      return itemIds
    }
    return []
  }
  ...
  absolutizeItemPath(itemPath) {
    return `${this.configFile.getParent().getPath()}/${itemPath}`
  }
  ...
  async save() {
    ...
    const isCreated = await this.configFile.create()
    return this.saveOnly()
  }
